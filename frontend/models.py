...
JOB_ACTIONS = (('SYNC', 'Sync'), ('INDEX', 'Index'), ('DEL', 'Delete'))
JOB_STATES = (('PEND', 'Pending'), ('PROG', 'In Progress'), ('COMP', 'Completed'), ('FAIL', 'Failed'))
...
class Client(models.Model):
    ...
    packages = models.ManyToManyField(Package, through='ClientPackageAvailability', blank=True)
    package_files = models.ManyToManyField(File, through='ClientFileAvailability', blank=True)
...
class Job(models.Model):
    ...
    def __str__(self):
        ...
        return "{0} - {1} - {2} - {3} -> {4}".format(state, action, self.package, self.source_client,
